...
if __name__ == '__main__':
	while True:
		...
		if re.match(r'instance,\d+,\d+,\d+,\d+$',inp) : 
			event_1vDetected = int(int(list_inp[1])) 
			event_1vGroundTruth = int(int(list_inp[2])) 
			event_1steering = int(int(list_inp[3])) 
			event_1speed = int(int(list_inp[4])) 
			event_1correctDetection = event_1vDetected == event_1vGroundTruth
			event_1deltaSteering = abs(event_1steering - ite(PAST_event_1steering!= None,PAST_event_1steering,event_1steering))
